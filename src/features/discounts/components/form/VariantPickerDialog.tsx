...
import { useDiscountVariantOptions } from '../../hooks';
import { DiscountItemValueType } from '../../types';
import type { DiscountVariantItemFormValues } from '../../schemas';
import { formatDiscountCurrency } from '../../utils/formatters';
...
const VariantPickerDialogComponent = ({
  open,
  onOpenChange,
  existingVariantIds,
  onAdd,
  ...
  const handleAdd = () => {
    const items: DiscountVariantItemFormValues[] = rows
      .filter((v) => selectedIds.includes(v.id))
      .map((v) => ({
        key: `variant-${v.id}-${Date.now()}`,
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-h-[80vh] overflow-hidden sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Pilih satu atau lebih varian untuk ditambahkan ke diskon
          </DialogDescription>
        </DialogHeader>
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        {variant.price != null ? (
                          <span className="font-mono text-muted-foreground">
                            {formatDiscountCurrency(variant.price)}
                          </span>
                        ) : null}
                        {already ? (
...
export const VariantPickerDialog = memo(VariantPickerDialogComponent);
