...
import { useDiscountProductOptions } from '../../hooks';
...
const ProductPickerDialogComponent = ({
  open,
  onOpenChange,
  existingVariantIds,
  onAdd,
    ...
    rows
      ...
      .forEach((product) => {
        (product.variants ?? []).forEach((variant) => {
          ...
          items.push({
            key: `prod-${product.id}-var-${variant.id}-${Date.now()}`,
  ...
  const totalVariantsToAdd = rows
    ...
    .reduce(
      (sum, p) =>
        sum +
        (p.variants ?? []).filter((v) => !existingSet.has(v.id)).length,
      0,
    );
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-h-[80vh] overflow-hidden sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Semua varian dari produk yang dipilih akan ditambahkan ke diskon
          </DialogDescription>
        </DialogHeader>
...
export const ProductPickerDialog = memo(ProductPickerDialogComponent);
